...
const UpdateGroupCaseQueuePermissionJuntion: React.FC<Props> = ({ initial }) => {
  ...
  const mutation = usePutApiGroupCaseQueuePermissionJuntionsId();
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (form.id) mutation.mutate({ id: form.id, data: form });
  };
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      {mutation.isError && <div>Error updating</div>}
      {mutation.isSuccess && <div>Updated successfully</div>}
    </form>
  );
};
